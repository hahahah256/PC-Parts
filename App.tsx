...
const App: React.FC = () => {
  ...
  const confirmAndBuild = useCallback(async () => {
    if (!validateForm() || !selectedGame) return;
    ...
    try {
      ...
      // Send lead to webhook automatically
  ...
  }, [userContact, selectedGame, budget, language, isRTL, validateForm]);
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
            <div className="bg-slate-900 border border-white/10 w-full max-w-md rounded-[3rem] p-10 shadow-2xl animate-in slide-in-from-bottom-12 duration-500">
              ...
              <div className="flex gap-4 mt-12">
                ...
                <button onClick={confirmAndBuild} className="flex-[2] py-4 bg-cyan-500 text-white font-black rounded-2xl shadow-xl shadow-cyan-500/20 hover:bg-cyan-400 transition-all">
                  {isRTL ? 'تأكيد وإنشاء' : 'Confirm & Build'}
                </button>
              </div>
            </div>
          </div>
              ...
              <div className="flex-1 w-full space-y-10">
                ...
                <div className="relative pt-6">
                  ...
                  <div className="flex justify-between mt-6">
                    <span className="text-xs font-black text-slate-700">60K {t.currency}</span>
                    ...
                    <div className="p-4 md:p-10 space-y-4">
                      {recommendation.parts.map((part, i) => (
                        <React.Fragment key={i}>
                          ...
                          {/* Gaming Case Upgrade Option */}
                          {(part.category.toLowerCase().includes('case')) && recommendation.gamingCaseUpgrade && (
                            <div className="ml-6 md:ml-12 p-6 bg-cyan-500/5 border border-cyan-500/20 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-6 animate-in slide-in-from-left duration-500">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <p className="text-[10px] font-black text-cyan-500 uppercase tracking-widest mb-0.5">{isRTL ? 'ترقية اختيارية' : 'Optional Upgrade'}</p>
                                  <h5 className="text-lg font-black text-white">{recommendation.gamingCaseUpgrade.name}</h5>
                                </div>
                              </div>
                              <div className="flex items-center gap-4">
                                <span className="text-sm font-black text-slate-500">+{recommendation.gamingCaseUpgrade.additionalCost.toLocaleString()} {t.currency}</span>
                                <button 
                                  onClick={() => {
                                    const lead: LeadPayload = {
                                      customer: userContact,
                                    ...
                                    const link = formatWhatsAppLink(SYSTEM_WHATSAPP, lead, language);
                                    window.open(`${link}%0A%0A*Upgrade:* ${recommendation.gamingCaseUpgrade?.name}`, '_blank');
                                  }}
                                  className="bg-cyan-500 hover:bg-cyan-400 text-white px-6 py-2 rounded-xl font-black text-xs uppercase tracking-widest transition-all"
                                >
                                  {isRTL ? 'اطلب مع الترقية' : 'Order with Upgrade'}
                                </button>
                              </div>
                            </div>
                          )}
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="p-6 bg-slate-800/50 rounded-3xl border border-white/5">
                        <p className="text-slate-400 text-sm leading-relaxed">
                          {isRTL ? 'هل لديك أسئلة؟ خبراؤنا هنا لمساعدتك في اختيار أفضل العتاد.' : 'Have questions? Our experts are here to help you choose the best hardware.'}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-6">
